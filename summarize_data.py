...
#     weather_df.drop(columns=['Climate ID','Year', 'Month', 'Day', 'Time (LST)','Temp Flag', 'Dew Point temp', 'Dew Point Temp Flag',
#        'Rel Hum (%)', 'Rel Hum Flag', 'Precip. Amount (mm)', 'Precip. Amount Flag', 'Stn Press (kPa)', 'Stn Press Flag'], inplace=True)
...
def summarize_data(raw_df, weather_df, granularity='hourly'):
    ...
    #                    'visibility_km','visibility_flag', 'hmdx', 'hmdx_flag', 'wind_chill', 'wind_chill_flag', 'weather']
    columns_to_drop = ['Climate ID', 'Year', 'Month', 'Day', 'Time (LST)', 'Temp Flag', 'Dew Point Temp (°C)', 
    ...
    # Select required columns
    selected_columns = ['Longitude (x)', 'Latitude (y)', 'Date/Time (LST)', 'Temp (°C)', 'Station Name']
    # selected_columns = ['longitude', 'latitude', 'date_time', 'temp', 'station_name']
    weather_df = weather_df[selected_columns]
    ...
    # Merge DataFrames
    merged_df = pd.merge(weather_df, raw_df_hourly, left_on='Date/Time (LST)', right_index=True, how='right')
    merged_df.rename(columns={'Date/Time (LST)': 'Date'}, inplace=True)
