...
WHERE e.reading NOT IN ('NaN')  AND e.reading > 0
...
"""
temp_observed_data_summary = "WITH quartiles AS (SELECT percentile_cont(0.25) WITHIN GROUP (ORDER BY temp) AS Q1, percentile_cont(0.75) WITHIN GROUP (ORDER BY temp) AS Q3 FROM weather_data_records WHERE temp NOT IN ( 'NaN') AND station_id = {} GROUP BY station_id), outlier_ranges AS (SELECT Q1, Q3, (Q3 - Q1) AS IQR, (Q1 - {} * (Q3 - Q1)) AS lower_bound, (Q3 + {} * (Q3 - Q1)) AS upper_bound FROM quartiles) SELECT 'TEMPERATURE' AS meter_name, COUNT(w.id) AS total_records, TO_CHAR(MIN(w.date_time), 'YYYY/MM/DD HH24:MI') AS time_stamp_start, TO_CHAR(MAX(w.date_time), 'YYYY/MM/DD HH24:MI') AS time_stamp_end,w.station_id AS meter_id, 'weather' as m_id FROM weather_data_records w JOIN outlier_ranges r ON w.temp NOT IN ( 'NaN') AND w.temp >= r.lower_bound AND w.temp <= r.upper_bound AND w.station_id = {} WHERE w.date_time between '{}' and '{}'  GROUP BY w.station_id;"

temp_observed_data_summary_list = "WITH quartiles AS (SELECT percentile_cont(0.25) WITHIN GROUP (ORDER BY temp) AS Q1, percentile_cont(0.75) WITHIN GROUP (ORDER BY temp) AS Q3 FROM weather_data_records WHERE temp NOT IN ( 'NaN') AND station_id = {meter_id} GROUP BY station_id), outlier_ranges AS (SELECT Q1, Q3, (Q3 - Q1) AS IQR, (Q1 - {METER_FACTOR} * (Q3 - Q1)) AS lower_bound, (Q3 + {METER_FACTOR} * (Q3 - Q1)) AS upper_bound FROM quartiles) SELECT TO_CHAR(w.date_time, 'YYYY/MM/DD HH24:MI') AS start_date, TO_CHAR(w.date_time + interval '1 hour', 'YYYY/MM/DD HH24:MI') AS end_date, w.station_id as meter_id, 104 as meter_type, temp as reading FROM  weather_data_records w WHERE w.station_id = {meter_id} AND w.temp NOT IN ( 'NaN') AND w.temp >= (select lower_bound from outlier_ranges ) AND w.temp <=  (select upper_bound from outlier_ranges ) {query_date_filter} order by start_date asc LIMIT {page_size} OFFSET ({page_number} - 1) * {page_size};"

outlier_summary_lower_bound_list= "WITH quartiles AS (SELECT meter_id, percentile_cont(0.25) WITHIN GROUP (ORDER BY reading) AS Q1, percentile_cont(0.75) WITHIN GROUP (ORDER BY reading) AS Q3 FROM epp.meter_hourly_entries WHERE reading NOT IN ( 'NaN') AND facility_id = {facility_id} and meter_type= {meter_type} AND meter_id = {meter_id} GROUP BY meter_id), outlier_ranges AS (SELECT (Q1 - {METER_FACTOR} * (Q3 - Q1)) AS lower_bound, (Q3 + {METER_FACTOR} * (Q3 - Q1)) AS upper_bound, meter_id FROM quartiles) SELECT TO_CHAR(start_date, 'YYYY/MM/DD HH24:MI') as start_date, TO_CHAR(end_date, 'YYYY/MM/DD HH24:MI') as  end_date, meter_id, meter_type, reading FROM epp.meter_hourly_entries e WHERE e.reading NOT IN ('NaN') AND e.reading > 0 AND e.reading < (SELECT lower_bound FROM outlier_ranges) AND e.is_independent_variable = {is_independent_variable} AND facility_id = {facility_id} and meter_type= {meter_type} AND meter_id = {meter_id}  {query_date_filter} order by start_date LIMIT {page_size} OFFSET ({page_number} - 1) * {page_size};"
outlier_summary_upper_bound_list= "WITH quartiles AS (SELECT meter_id, percentile_cont(0.25) WITHIN GROUP (ORDER BY reading) AS Q1, percentile_cont(0.75) WITHIN GROUP (ORDER BY reading) AS Q3 FROM epp.meter_hourly_entries WHERE reading NOT IN ( 'NaN') AND facility_id = {facility_id} and meter_type= {meter_type} AND meter_id = {meter_id} GROUP BY meter_id), outlier_ranges AS (SELECT (Q1 - {METER_FACTOR} * (Q3 - Q1)) AS lower_bound, (Q3 + {METER_FACTOR} * (Q3 - Q1)) AS upper_bound, meter_id FROM quartiles) SELECT TO_CHAR(start_date, 'YYYY/MM/DD HH24:MI') as start_date, TO_CHAR(end_date, 'YYYY/MM/DD HH24:MI') as  end_date, meter_id, meter_type, reading FROM epp.meter_hourly_entries e WHERE e.reading NOT IN ( 'NaN')  AND e.reading > 0 AND e.reading > (SELECT upper_bound FROM outlier_ranges) AND e.is_independent_variable = {is_independent_variable} AND facility_id = {facility_id} and meter_type= {meter_type} AND meter_id = {meter_id} {query_date_filter} order by start_date LIMIT {page_size} OFFSET ({page_number} - 1) * {page_size};"
...
missing_data_summary = """WITH date_range AS (
    ...
    JOIN meter_details AS md ON md.id= e.meter_id
	WHERE facility_id = {facility_id} and meter_type= {meter_type} AND ( reading = 'NaN' OR reading = 0 or reading IS NULL OR reading < 0) 
		AND start_date >= (SELECT MAX(start_date) from date_range where meter_id > 0) 
		AND end_date <=  (SELECT MIN(end_date) from date_range where meter_id > 0) 
		AND is_independent_variable = {is_independent_variable}
        {date_filter}
...
UNION
SELECT meter_type, meter_name, m_id, total_records, TO_CHAR(time_stamp_start::timestamp, 'YYYY/MM/DD HH24:MI') as time_stamp_start, TO_CHAR(time_stamp_end::timestamp, 'YYYY/MM/DD HH24:MI') as time_stamp_end, meter_id, 1 as missing_type FROM missing_date_range WHERE total_records > 0"""
...
temp_missing_data_summary_list = "WITH quartiles AS (SELECT  meter_id, MAX(end_date) as end_date, MIN(start_date) as start_date FROM epp.meter_hourly_entries WHERE reading NOT IN ( 'NaN') AND facility_id = {facility_id} and meter_type= {meter_type} GROUP BY  meter_id) SELECT TO_CHAR(w.date_time, 'YYYY/MM/DD HH24:MI') AS start_date, TO_CHAR(w.date_time + interval '1 hour', 'YYYY/MM/DD HH24:MI') AS end_date, w.station_id as meter_id, 104 as meter_type, temp as reading FROM  weather_data_records w WHERE w.station_id = {meter_id} AND (temp = 'NaN' OR temp IS NULL) AND date_time >= (SELECT MAX(start_date) from quartiles where meter_id > 0) AND date_time <= (SELECT MIN(end_date) from quartiles where meter_id > 0) {query_date_filter} order by start_date asc LIMIT {page_size} OFFSET ({page_number} - 1) * {page_size};"
...
WITH MeterReadings AS (
    ...
    WHERE 
        mhe.facility_id = {}  -- Moved condition up from JOIN
        AND mhe.reading > 0
),
AllMetricsQuartiles AS (
    -- Combine all metrics calculations into one CTE
    SELECT 
    ...
    FROM (
        SELECT ROUND(AVG(temp)::numeric, 2) AS avg_temp
        FROM epp.weather_data_records
        WHERE station_id IN (51459, 31688, 54958)
        GROUP BY date_time
    ) temp_data
    ...
    SELECT 
        ...
        PERCENTILE_CONT(0.25) WITHIN GROUP (ORDER BY avg_reading) AS Q1,
        PERCENTILE_CONT(0.75) WITHIN GROUP (ORDER BY avg_reading) AS Q3
    FROM (
        SELECT ROUND(AVG(reading)::numeric, 2) AS avg_reading
        FROM epp.meter_hourly_entries
        WHERE facility_id = {} 
        AND reading >= 0 
...
SELECT 
    meter_name,
    ROUND((Q3 - Q1)::numeric, 2) as inter_quartile,
    ROUND(Q1::numeric, 2) as first_quartile,
    ROUND(Q3::numeric, 2) as third_quartile,
    ROUND((Q1 - CASE 
        WHEN meter_name = 'Independent Variable' THEN 1.5 
        ELSE 3 
    END * (Q3 - Q1))::numeric, 2) AS lower_limit,
    ROUND((Q3 + CASE 
        WHEN meter_name = 'Independent Variable' THEN 1.5 
        ELSE 3 
    END * (Q3 - Q1))::numeric, 2) AS upper_limit
FROM AllMetricsQuartiles
ORDER BY meter_name;
"""
...
#     Quartiles2;
# """
daterangequery="With quartiles as (SELECT  meter_id, MAX(end_date) as end_date, MIN(start_date) as start_date FROM epp.meter_hourly_entries WHERE reading NOT IN ( 'NaN') AND facility_id = {facility_id} and meter_type= {meter_type} GROUP BY  meter_id) SELECT MAX(start_date) as start_date, MIN(end_date) as end_date FROM quartiles WHERE meter_id > 0"
