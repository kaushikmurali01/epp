...
class AuthorizationService {
  public static async check(
    ...
    try {
      ...
      const companyRole: any = await UserCompanyRole.findOne({
        where: {
  ...
  public static async checkFaciltiy(
    facilityId: number,
    email: number
  ): Promise<any> {
    try {
      if (facilityId) {
        let checkPermission = await UserResourceFacilityPermission.findOne({
          where: { email: email, facility_id: facilityId },
        });
        return checkPermission && checkPermission.id ? true : false;
      } else {
        return false;
      }
    } catch (error) {
