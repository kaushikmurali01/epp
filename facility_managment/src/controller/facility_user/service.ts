 ...
 `);
   ...
   c.company_name
     ...
     left JOIN 
         company c ON f.company_id = c.id
     WHERE 
     f.facility_id_submission_status NOT IN (${
       FACILITY_ID_SUBMISSION_STATUS.DRAFT
     },${FACILITY_ID_SUBMISSION_STATUS.READY_FOR_SUBMISSION},
       ${FACILITY_ID_SUBMISSION_STATUS.IN_REVIEW},${
            FACILITY_ID_SUBMISSION_STATUS.APPROVED
          },${FACILITY_ID_SUBMISSION_STATUS.REJECTED},
       ...
       ${
         FACILITY_ID_SUBMISSION_STATUS.ENROLLMENT_IN_PROGRESS_APPROVEL_PENDING
       }) AND
 ...
 `);
   ...
   c.company_name
     ...
     left JOIN 
         company c ON f.company_id = c.id
     WHERE 
     f.facility_id_submission_status IN (${
       FACILITY_ID_SUBMISSION_STATUS.DRAFT
     },${FACILITY_ID_SUBMISSION_STATUS.READY_FOR_SUBMISSION},
       ${FACILITY_ID_SUBMISSION_STATUS.IN_REVIEW},${
            FACILITY_ID_SUBMISSION_STATUS.APPROVED
    ...
    try {
      ...
      const findPermission = await UserCompanyRolePermission.findOne({
        where: {
          user_id: userToken.id,
          company_id: facility.company_id,
          permission_id: { [Op.in]: [15, 16] }, // do 7,8 for dev env becuase permission are different
        },
      });
