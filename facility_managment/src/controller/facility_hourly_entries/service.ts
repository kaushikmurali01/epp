...
export class FacilityMeterHourlyEntriesService {
  ...
  static async getFacilityMeterEntriesListing(userToken: IUserToken, offset:number, limit:number, colName:string, order:string, facilityMeterId:number): Promise<FacilityMeterHourlyEntries[]> {
    try {
      const result = await FacilityMeterHourlyEntries.findAndCountAll({where:{facility_meter_detail_id: facilityMeterId, is_active:STATUS.IS_ACTIVE},offset:offset, limit:limit, order: [[colName, order]]})    
      if(result){
  ...
  static async createNewMeterEntry(userToken: IUserToken, body:IBaseInterface): Promise<FacilityMeterHourlyEntries[]> {
      try {
        const obj = {
  ...
  static async editMeterDetails(userToken: IUserToken, body:IBaseInterface, facilityId:number): Promise<FacilityMeterHourlyEntries[]> {
    try {
      const obj = {
        facility_id: body.facility_id,
        facility_meter_detail_id: body.facility_meter_detail_id,
      ...
      const result = await FacilityMeterHourlyEntries.update(obj,{where:{id:facilityId}})
  ...
  static async removeMeter(userToken: IUserToken, facilityId:number): Promise<FacilityMeterHourlyEntries[]> {
    try {
      const result = await FacilityMeterHourlyEntries.update({is_active:STATUS.IS_DELETED} ,{where:{id:facilityId}})
      const resp = ResponseHandler.getResponse(HTTP_STATUS_CODES.SUCCESS, RESPONSE_MESSAGES.Success, result);
      return resp;
