...
export class FacilityMeasureController {
   ...
   */
  static async getFacilityMeasureById(decodedToken: IUserToken, event: HttpRequest): Promise<FacilityMeasure[]> {
    try {
      const result = await FacilityMeasureService.getFacilityMeasureById(Object(decodedToken), Number(event.params.id));
      return result
    } catch (error) {
      this.errorMessage = {
        message: error,
   ...
   * @returns {object[]} response_data - API Response Data
   */
  static async addFacilityMeasure(decodedToken: IUserToken, body: IBaseInterface): Promise<FacilityMeasure[]> {
    try {
      // await validatorApiResponse(addFacilityCharSchema, body);
      const result = await FacilityMeasureService.addFacilityMeasure(Object(decodedToken), body);
      return result
    } catch (error) {
      this.errorMessage = {
        message: error,
