...
class CompanyController {
  ...
  static async listCompanies(
    ...
    try {
      const [count, rows] = await CompanyService.listCompanies(
        offset,
        limit,
        searchPromt,
        companyFilter,
...
static async GetCompanyUser(offset, limit, company_id): Promise<any> {
  ...
  static async UpdateCompanyStatus(companyId, req): Promise<any> {
    try {
      const updatedData = req;
      const findData: any =
        await rawQuery(`select "u"."first_name","u"."email","c"."company_name" from "users" as "u" INNER join "user_company_role" as ucr on
      ...
      template = template
        .replace(
          "#heading#",
          "Company Edited for Energy Performance Program Portal"
        )
        .replace(
          "#content#",
          `
