...
export async function UpdateCompanyStatus(
  ...
  try {
    ...
    const company = await CompanyController.UpdateCompanyStatus(
      id,
      requestData
    );
 ...
 */
export async function sendAlertForCompany(
  request: HttpRequest,
  context: InvocationContext
): Promise<HttpResponseInit> {
  try {
    ...
    // }
    const requestData = await request.json();
    const company = await CompanyController.sendAlertForCompany(
      requestData,
      id
    );
...
export async function ListCompanies(
  ...
  try {
    ...
    // Get all companies
    const companies = await CompanyController.listCompanies(
      pageOffset,
      pageLimit,
      searchPromt,
      companyFilter,
...
export async function getCompanyUser(
  ...
  try {
    ...
    const id = resp.id;
    const companies = await CompanyController.getCompanyUser(
      id,
      company_id,
      resp.role_id
    );
 ...
 * @returns A promise resolving to an HTTP response containing all companies.
 */
export async function changeSuperUserForCompany(
  request: HttpRequest,
  context: InvocationContext
): Promise<HttpResponseInit> {
  try {
    ...
    const id = resp.id;
    const companies = await CompanyController.changeCompanySuperUser(
      id,
      company_id,
      user_id,
      resp.role_id
 ...
 * @returns A promise resolving to an HTTP response containing all companies.
 */
export async function DropDownCompanies(
  request: HttpRequest,
  context: InvocationContext
): Promise<HttpResponseInit> {
  try {
...
export async function GetCompanyAdmin(
  ...
  try {
    ...
    //     return { status: 401, body: RESPONSE_MESSAGES.notFound404 };
    // }
    await decodeTokenMiddleware(request, context, async () =>
      Promise.resolve({})
    );
...
export async function CheckCompanyByName(
  request: HttpRequest,
  context: InvocationContext
): Promise<HttpResponseInit> {
  try {
...
export async function CheckCompanyStatus(
  ...
  try {
    ...
    const company = await Company.findOne({
      where: { id: company_id, is_active: 1 },
    });
    if (company) {
      return true;
    } else {
...
export async function MakeCompanyInActive(
  request: HttpRequest,
  context: InvocationContext
): Promise<HttpResponseInit> {
  const requestData = await request.json();
