...
class CompanyService {
    ...
    try {
      const adminUsers = await User.findOne({
        ...
        attributes: [
          ...
          [
            sequelize.col("UserCompanyRole.Company.company_name"),
            "company_name",
          ],
          [
            sequelize.col("UserCompanyRole.Company.company_type"),
            "company_type",
          ],
  ...
  static async DropDownCompanies(): Promise<any[]> {
    try {
      let result = await Company.findAll({
        attributes: ["company_name", "id", "is_active"],
      });
      return result;
    } catch (error) {
      throw error;
  ...
  static async checkExistSuperAdmin(user_id, company_id): Promise<any> {
    try {
      let result = await UserCompanyRole.findOne({
        where: { is_active: 1, user_id, company_id, role_id: 1 },
      });
      return result;
    } catch (error) {
      throw error;
  ...
  static async SendEmailsToAllAdmins(adminData, title, body): Promise<any> {
    ...
    let contentAdmin = body
      .replace("#name#", adminData?.superAdmin?.name)
