...
from sql_queries.weather_station_queries import UPDATE_IN_USE, MISSING_TEMPERATURE_QUERY
...
def upload_chunk(conn, chunk, station_id):
    ...
    try:
        ...
        # Log the DataFrame shape or other details
        print(f"DataFrame prepared with {len(chunk)} rows.")

        # Create a temporary staging table
        cur.execute("""
            CREATE TEMP TABLE IF NOT EXISTS temp_weather_data_records ON COMMIT DROP AS
            SELECT * FROM epp.weather_data_records WITH NO DATA;
        """)
        ...
        cur.copy_expert(sql.SQL("""
            COPY temp_weather_data_records (longitude, latitude, station_name, climate_id, date_time, year, month, day, time, 
        ...
        # Perform a bulk update from the temporary table
        cur.execute("""
            UPDATE epp.weather_data_records AS main
            SET longitude = temp.longitude, latitude = temp.latitude, station_name = temp.station_name, 
                climate_id = temp.climate_id, year = temp.year, month = temp.month, day = temp.day, time = temp.time, 
                temp = temp.temp, temp_flag = temp.temp_flag, dew_point_temp = temp.dew_point_temp, 
                dew_point_temp_flag = temp.dew_point_temp_flag, rel_hum = temp.rel_hum, rel_hum_flag = temp.rel_hum_flag, 
                precip_amount = temp.precip_amount, precip_amount_flag = temp.precip_amount_flag, wind_dir = temp.wind_dir, 
                wind_dir_flag = temp.wind_dir_flag, wind_spd = temp.wind_spd, wind_spd_flag = temp.wind_spd_flag, 
                visibility_km = temp.visibility_km, visibility_flag = temp.visibility_flag, station_press = temp.station_press, 
                station_press_flag = temp.station_press_flag, hmdx = temp.hmdx, hmdx_flag = temp.hmdx_flag, 
                wind_chill = temp.wind_chill, wind_chill_flag = temp.wind_chill_flag, weather = temp.weather
            FROM temp_weather_data_records temp
            WHERE main.date_time = temp.date_time AND main.station_id = temp.station_id;
        """)
        conn.commit()
        print("Update successful.")
        return f"Successfully updated data for station {station_id}"
    except psycopg2.Error as e:
        conn.rollback()
        print(f"Database error: {e}")
        return f"Error while updating data for station {station_id}: {str(e)}"
    except Exception as e:
        conn.rollback()
        print(f"Unexpected error: {e}")
        return f"Unexpected error while updating data for station {station_id}: {str(e)}"
    finally:
        cur.close()
...
#     except Exception as e:
#         conn.rollback()
#         print(f"Error while updating data for station {station_id}: {str(e)}")
#         return f"Error while updating data for station {station_id}: {str(e)}"
#     finally:
#         cur.close()
...
def process_station(station_id, year, month, day, time_frame):
    try:
        ...
        lookup_date = datetime.datetime.now() - datetime.timedelta(days=15)
        chunk_reader['Date/Time (LST)'] = pd.to_datetime(chunk_reader['Date/Time (LST)'])
        chunk_reader = chunk_reader[chunk_reader['Date/Time (LST)'] <= lookup_date]
...
def main(stations=None, in_use=False):
    missing_data = dbtest(MISSING_TEMPERATURE_QUERY)
...
if __name__ == "__main__":
    """
    This CRON JOB will look for the missing temperature data in last 365 days 
    and update it in the database so that we can fill out the missing values.
    """
    main()
