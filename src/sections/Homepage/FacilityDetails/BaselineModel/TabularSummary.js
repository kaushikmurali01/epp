...
import { fetchBaselinePredictedData } from "../../../../redux/superAdmin/actions/baselineAction";
...
const TabularSummary = ({ meterType }) => {
  ...
  const count = useSelector(
    (state) => state?.baselineReducer?.baselinePredictedData?.count || []
  );
  const tableData = useSelector(
    (state) => state?.baselineReducer?.baselinePredictedData?.data || []
  );
  const user_id =
