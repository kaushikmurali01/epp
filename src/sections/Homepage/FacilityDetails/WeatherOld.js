  ...
  const Weather = () => {
    ...
    const getWeatherData = async () => {
      ...
      try {
        ...
        const [tempRes, relHumRes, precipRes, windRes, pressRes] =
          await Promise.all([
            GET_REQUEST(
              WEATHER_INDEPENDENT_VARIABLE_ENDPOINTS.GET_WEATHER_DATA +
                `?facility_id=${facilityData?.id}&data=${nameMap[0]}`
            ),
            GET_REQUEST(
              WEATHER_INDEPENDENT_VARIABLE_ENDPOINTS.GET_WEATHER_DATA +
                `?facility_id=${facilityData?.id}&data=${nameMap[1]}`
            ),
            GET_REQUEST(
              WEATHER_INDEPENDENT_VARIABLE_ENDPOINTS.GET_WEATHER_DATA +
                `?facility_id=${facilityData?.id}&data=${nameMap[2]}`
            ),
            GET_REQUEST(
              WEATHER_INDEPENDENT_VARIABLE_ENDPOINTS.GET_WEATHER_DATA +
                `?facility_id=${facilityData?.id}&data=${nameMap[3]}`
            ),
            GET_REQUEST(
              WEATHER_INDEPENDENT_VARIABLE_ENDPOINTS.GET_WEATHER_DATA +
                `?facility_id=${facilityData?.id}&data=${nameMap[4]}`
            ),
          ]);
