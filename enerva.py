...
class EnergyModel:
    ...
    def create_dummies(self, column_names):
        """
        Convert specified categorical columns into dummy variables if they are not binary,
        and return a list of new dummy variables created.
        
        Parameters:
        - dataframe: pandas.DataFrame containing the data.
        - column_names: list of str, names of the columns to check and transform.
        
        Returns:
        - Updated DataFrame with dummy variables.
        - List of new dummy variable names created.
        """
        new_dummy_vars = []
    ...
    def training_hourly_model_sm(self):
        ...
        #     # Save the model summary (including R2 and other metrics)
        #     summary_table_0 = model.summary2().tables[0]
        #     summary_table_1 = model.summary2().tables[1]
        #     self.model_summaries[self.model_segment_name_mapping[segment_name]] = pd.concat([summary_table_0, summary_table_1])
    
        # # Saving the model summaries to a single Excel file with different sheets
        # with pd.ExcelWriter('model_summaries.xlsx') as writer:
        #     for segment_name, summary_df in self.model_summaries.items():
        #         summary_df.to_excel(writer, sheet_name=f'summary_{segment_name}')
    
        # Displaying the results for the three_month_weighted segments
        # for segment_name in self.model_segment_name_mapping.values():
        #     # print(f"Segment: {segment_name}")
        #     # print("Coefficients:", self.feature_coefficients[segment_name])
        #     # print("Intercept:", self.intercept[segment_name])
        #     # print("Regression Formula:", self.regression_formula[segment_name])
        #     # print("P-values:", self.p_values[segment_name])
        #     # print("T-statistics:", self.t_stats[segment_name])
        #     print("Model Summary:\n", self.model_summaries[segment_name])
        #     print("\n")
    ...
    def training_daily_model(self, processed_baseline_data, ignore_disqualification=False):
        ...
        baseline_predictions['Residual'] = baseline_predictions['observed'] - baseline_predictions['predicted']
        # self.result = result
    ...
    def evaluate(self,scoring_data):
        ...
        results = {
            ...
            "Coefficient of Determination, R2": 100*r2,
            "Adjusted R2": 100*adj_r2,
            "Root-mean-square error, RMSE R2": rmse,
            "Coefficient of variation of RMSE": f"{cv_rmse:.2f}%",
            "Auto correlation function": autocorr,
            "Durbin-Watson (P>0)": dw_stat
        }
    ...
    def plot_results(self):
        ...
        plt.plot(self.data.index, self.data['observed'], label='Observed')
        plt.plot(self.data.index, self.data['predicted'], label='Predicted', alpha=0.7)
        plt.legend()
